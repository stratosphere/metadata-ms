...
public class FlinkWriteConstraintTest {
    ...
    public void testInclusionDependency() throws Exception {
    	...
    	inds.add(new Tuple2<int[], int[]>(referenced, dependent));
        ...
        assertTrue( ( (InclusionDependency) store.getConstraintCollections().iterator().next().getConstraints().iterator().next())
        				.getTargetReference().getReferencedColumns() == inds.get(0).f0 );
